...
## Step-by-step: install the wheel from the release
...
- `smoke_pyarrow.py` and `smoke_datasets.py` write scratch parquet files in the current working directory.
...
| Python | CPython 3.11.15 ARM64 (via uv), venv at `C:\Users\you\.venv-arm64` with native `torch 2.12.1+cpu` already installed |
...
| Downstream training dry-run (first cycle) native | PASS (param parity below) |
| Other 4 downstream training scripts import datasets | PASS (blocked only on pre-cycle artifacts, env-independent) |
...
- **First downstream training cycle (`<training_script.py> --dry-run`): PASSED** with `trainable params: 113,770,496 || all params: 1,833,798,656 || trainable%: 6.2041` — exact parity with the WSL2 and x86-emulated runs. The dry-run data path used real `datasets.map` (1 example, tokenized) and the forward pass returned `loss=2.527240753173828`, logits `[1,164,151669]`.
- The other four training scripts all import `datasets` successfully (zero ImportErrors) and fail only on missing pipeline artifacts that any environment — including WSL — would hit before the first training cycle completes (empty adapter dir → `Can't find 'adapter_config.json'`; a missing `<training_data>.jsonl` that the script itself tells you to generate first).
...
- **cp311-only wheel.** This build targets CPython 3.11 ARM64 because that is the Python version used for the verified install; other versions need their own build.
- **Windows ARM64 only.** The wheel is built for `win_arm64`; it will not install on x86_64 or other platforms.
- **No bz2 codec in pyarrow.** `ARROW_WITH_BZ2=OFF` is required because bundled bzip2 does not build under Ninja (§5). Plain `.bz2` files are still handled by Python's stdlib `bz2`; only bz2-compressed parquet is affected.
- **No S3/GCS/Azure/Flight/Gandiva/ORC/Substrait.** These features are OFF to keep the build focused on `datasets`/`trl`. Enable them only if you are willing to build their dependency trees from source.
- **MSVC ARM64 toolchain required.** The scripts assume Visual Studio 2022 Build Tools with the native ARM64 compiler and the Windows SDK. Cross-compiling from an x64 host is a different exercise.
- **One machine, one day.** All numbers are from a single Snapdragon X Elite session; re-run `smoke_pyarrow.py` and your own workload to validate on your hardware.

---

## 10. Reproduce this build
...
4. **Edit placeholders.** The `.bat` scripts use `C:\Users\you\...` placeholders at the top (`BASE`, `NINJADIR`, `PYEXE`). Point them at your own build directory, ninja directory, and ARM64 Python 3.11 venv before launching. The smoke scripts write scratch files in the current directory.
5. **C++ stage:** launch `run_cpp_build.bat` (the pattern-B wrapper — never Start-Process with stream redirection, §4):

   ```powershell
   powershell Start-Process cmd.exe -ArgumentList '/c','C:\Users\you\GitHub_Repos\Windows_ARM64_PyArrow\run_cpp_build.bat' -WindowStyle Hidden
   ```
...
6. **Wheel stage:** same pattern with `run_pyarrow_wheel.bat`; watch `pyarrow_build.out.log` for `PYARROW_WHEEL_BUILD_OK` (~3 min). The repaired wheel lands in `%BASE%\arrow-25.0.0\python\repaired_wheels\`.
7. **Install and verify** per the step-by-step instructions above: `pip install` the repaired wheel, `smoke_pyarrow.py` → `PYARROW_SMOKE_OK`, then `pip install datasets trl` and `smoke_datasets.py` → `DATASETS_SMOKE_OK`.
